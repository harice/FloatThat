...
if( isset($tw_users_id) && $tw_users_id > 0)
{
		...
		$date=date('Y-m-d');
			...
			mysql_query("insert into tbl_deal(deal_id, pro_id,user_id,date,ods,type)values('".$_SESSION['deal_id']."', '$pro_id','$user','$date', '".$nmembers."','".$_POST['type']."')");
	...
	<input type="hidden" name="currency_code" value="USD">
		...
		<input type="hidden" name="amount" value="<?=number_format($amount,2)?>">
				...
				<script type="text/javascript">
					...
					setTimeout(function( ) { document.buypro.submit( ); }, 100); // comment to be removed

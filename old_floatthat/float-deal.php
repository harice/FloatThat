...
<tr>
	...
	<td>
	<?php 
		$fb_frnd=$facebook->api('/me/taggable_friends');
		// $fb_frnd=$facebook->api('/me/invitable_friends');

		if(!empty($fb_frnd)){
			foreach($fb_frnd['data']  as $k=>$vl){
			?>
			<div class='grid_float'>
				<label>
					<img src="<?php echo $vl['picture']['data']['url']; ?>" />
					<br/>
					<?php echo $vl['name']; ?>
					<br/>
					<input type='checkbox' name='fb_frnds[]' value='<?php echo $vl['id']; ?>' />
				</label>
			</div>
			<?php
			}
	...
	<div id="fb-root"></div> 
	<?php
		$link = "http://www.floatthat.net/product-details.php?id=".$_GET['id']."&from=13&deal_id=".$_SESSION['deal_id'] ;
	?>
<input type="button" value="send" onclick="sendFbMsg('<?=$link?>')" />
	</td>
</tr>
<tr>

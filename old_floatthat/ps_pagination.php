...
class PS_Pagination {
	...
	function PS_Pagination($sql, $rows_per_page = 10, $links_per_page = 5, $append = "", $page) {
	...
	//		return false;
		...
		$rs = $commonObject->selectMultiRecords($this->sql );	
		$this->total_rows = count($rs);//mysql_num_rows($all_rs );
		@mysql_close($all_rs );
		...
		$rs = $commonObject->selectMultiRecords( $this->sql . " LIMIT {$this->offset}, {$this->rows_per_page}" );
		if (! $rs) {
			if ($this->debug)
	 ...
	 * @return string
	 */
	function renderNav($prefix = '<a class="activePage" href="#">', $suffix = '</a>') {
		if ($this->total_rows == 0)
			return FALSE;

...
function float_deals()
{
	...
	$( "#by_my_deals_link" ).removeClass( "large_btn5" ).addClass( "large_btn" );
...
function group_deals()
{
	...
	$( "#group_deal_link" ).removeClass( "large_btn" ).addClass( "large_btn5" );
	$( "#by_my_deals_link" ).removeClass( "large_btn5" ).addClass( "large_btn" );
...
function my_deals()
{
	$( "#flaot_deal_link" ).removeClass( "large_btn5" ).addClass( "large_btn" );
	$( "#group_deal_link" ).removeClass( "large_btn5" ).addClass( "large_btn" );
	$( "#by_my_deals_link" ).removeClass( "large_btn" ).addClass( "large_btn5" );
...
<a class="large_btn" id="flaot_deal_link" onClick="float_deals();" href="#" style=" cursor:pointer;">My Float Deals</a>
<a class="large_btn" id="group_deal_link" onClick="group_deals();"  href="#" style=" cursor:pointer;">My Group Deals</a>
<a class="large_btn" id="by_my_deals_link" onClick="my_deals();"  href="#" style=" cursor:pointer;">My Buy Deals</a>
<a class="large_btn" id="by_my_deals_link" onClick="my_deals();"  href="#" style=" cursor:pointer;">My Products</a>
...
<div id="receivedd"  style="width:800px; border:1px solid #CCC; margin-top:20px;" align="center">
		...
		WHERE sales.uid = '".$userContents['id']."' and sales.status = 2";
...
<div id="receivedd"  style="width:800px; border:1px solid #CCC; margin-top:20px;" align="center">
			...
			<?php 
			//echo "select deal.*,pro.* from tbl_deal deal,tbl_products pro where deal.user_id=$user and pro.pro_id=deal.pro_id and deal.status=1 and deal.type = 'group'";mysql_query("delete from tbl_deals where user_id=$user and status=0");
		
			
			$mydealqry=mysql_query("select deal.*,pro.* , photos.image
			from tbl_deal deal,tbl_products pro 
			inner join tbl_photos as photos on photos.pro_id = pro.pro_id
			where deal.user_id=$user and pro.pro_id=deal.pro_id and deal.status=1 and deal.type = 'group' GROUP BY deal.pro_id, deal.deal_id");
		...
		if($rowss > 0)
			...
			while($dealinfo=mysql_fetch_array($mydealqry))
			{
				...
				$totparticpaneqry=mysql_query("select * from tbl_members where deal_id=$dealinfo[deal_id] where status=1");
				$totnummembers=mysql_num_rows($totparticpaneqry);
				
				
				$mparticpaneqry=mysql_query("select mem.*,user.* from tbl_members mem,user_info as user where mem.deal_id=$dealinfo[deal_id] and mem.status=0 and user.user_id=mem.user_id  ");
    ...
    padding:10px; padding:5px;margin:5px;" align="left">
         ...
         <img src="wadmin/gallaryimg/thumbnails/<?php echo $dealinfo['image'];?>" style="width:140px; height:85px; border:none" alt="product" /></a>  </div>
                ...
                <div style="float:left; width:100px;"><?php echo $maturemembers-$totnummembers;?></div>
...
$mparticpaneqry=@mysql_query("select mem.*,user.* from tbl_members mem 
...
<div id="receivedd"  style="width:800px; border:1px solid #CCC; margin-top:20px;" align="center">
			...
			<?php 
			//echo "select deal.*,pro.* from tbl_deal deal,tbl_products pro where deal.user_id=$user and pro.pro_id=deal.pro_id and deal.status=1 and deal.type = 'float'";mysql_query("delete from tbl_deals where user_id=$user and status=0");
			$mydealqry=mysql_query("select deal.*,pro.*, photos.image from tbl_deal deal,tbl_products pro 
			inner join tbl_photos as photos on photos.pro_id = pro.pro_id
			where deal.user_id=$user and pro.pro_id=deal.pro_id and deal.status=1 and deal.type = 'float' GROUP BY deal.pro_id, deal.deal_id");
		$rowss = mysql_num_rows($mydealqry);
		...
		if($rowss > 0)
			...
			{
				//$particpaneqry=mysql_query("select * from tbl_members where deal_id=$dealinfo[deal_id]");
				$particpaneqry=mysql_query("select * from invitations where deal_id=$dealinfo[deal_id]");
				...
				//$totparticpaneqry=mysql_query("select * from tbl_members where deal_id=$dealinfo[deal_id] where status=1");
				$totparticpaneqry=mysql_query("select * from invitations where deal_id=$dealinfo[deal_id] ");
				$totnummembers=mysql_num_rows($totparticpaneqry);
				//$mparticpaneqry=mysql_query("select mem.* from invitations mem where mem.deal_id=$dealinfo[deal_id]");
				$mparticpaneqry=mysql_query("select distinct mem.*   from tbl_members mem inner join invitations inv on inv.deal_id = mem.deal_id where mem.deal_id=$dealinfo[deal_id] and inv.deal_id=$dealinfo[deal_id] and mem.status=1");
				$maturemembers=mysql_num_rows($mparticpaneqry);
    ...
    padding:10px; padding:5px;margin:5px;" align="left">
         ...
         <img src="./wadmin/gallaryimg/thumbnails/<?php echo $dealinfo['image'];?>" style="width:140px; height:85px; border:none" alt="product" /></a>  </div>
                <div style="clear:both"></div>
                <div style="float:left; width:122px;">Total Cost :</div>
                <div style="float:left; width:100px;">$<?php echo number_format($dealinfo['price'],2);?></div>
                <div style="clear:both"></div>
                <div style="float:left; width:122px;">Total participant :</div>
                <div style="float:left; width:100px;"><?php echo $nummembers;?></div>
                <div style="clear:both"></div>
                <div style="float:left; width:122px;">Pending participant: </div>
                <div style="float:left; width:100px;"><?php echo $maturemembers-$totnummembers;?></div>
...
if($memberspic2['fb'] == 1)
           ...
           <?php /*?>     <a href="https://www.facebook.com/profile.php?id=<?php echo $memberspic2['user_id'];?>" target="_blank">
           <?php */?>     <?php echo "<b>".$i."</b> : ".$memberspic2['email'];?><br>
            <?php /*?>    <img src="<?php echo $memberspic2['friend_pics'];?>"  style="width:50px; height:50px;border:1px solid #069; padding:5px; margin:5px;" /></a></div>
            <?php */?>
            	</div>    
                <?php 
				}
				...
				?>
                <div style="margin-left:15px;">
                <?php echo $i.": ".$memberspic2['email'];?>
              <?php /*?>  <img src="./avatar/<?php echo $memberspic2['pic'];?>"  style="width:50px; height:50px;border:1px solid #069; padding:5px; margin:5px;" />
              <?php */?>  
                </div>
				...
				while($memberspic=mysql_fetch_array($mparticpaneqry))
				{
					$memId = $memberspic["mem_id"];
					
						

									if( isset($memberspic['fb']) && $memberspic['fb'] == 1)
					{
				?>
                <div style="float:left; width:50px; margin-left:15px;">
                Email:<?php echo $memberspic['friend_email'];?><br>
           <?php /*?>     <img src="<?php echo $memberspic['friend_pics'];?>"  style="width:50px; height:50px;border:1px solid #069; padding:5px; margin:5px;" />
           <?php */?>     </div>
                ...
                <div style="float:left; width:250px; margin-left:15px;">
                 Email:<?php echo $memberspic['friend_email'];?>
             <?php /*?>   <img src="./avatar/<?php echo $memberspic['pic'];?>"  style="width:50px; height:50px;border:1px solid #069; padding:5px; margin:5px;" />
             <?php */?>   </div>
